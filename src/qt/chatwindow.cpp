    ...
    : QWidget(parent), ui(new Ui::ChatWindowClass)
{
	...
	connect(ui->buttonConnecti2p, SIGNAL(clicked()), this, SLOT(connectei2p()));
...
void ChatWindow::connectei2p()
{
    ...
    ui->tab->addTab(textEdit,"Console/PM");
    ui->tab->setTabToolTip(ui->tab->count()-1,"127.0.0.1");
    // current tab is now the last, therefore remove all but the last
    for (int i = ui->tab->count(); i > 1; --i) {
       ui->tab->removeTab(0);
    }

    serveurs.insert("127.0.0.1",serveur);

	serveur->pseudo=ui->editPseudo->text();
    serveur->serveur="127.0.0.1";
    serveur->port=6668;
	serveur->affichage=textEdit;
    serveur->tab=ui->tab;
	serveur->userList=ui->userView;
    ...
    serveur->connectToHost("127.0.0.1",6668);
